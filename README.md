...
```bash
curl "http://localhost:3000/rules?enabled=true"
```

* **Update/Toggle a Rule** (`PATCH /rules/:id`)

```bash
curl -X PATCH http://localhost:3000/rules/REPLACE_WITH_ID \
  -H "Content-Type: application/json" \
  -d '{"enabled": false}'
```
...
curl -X POST http://localhost:3000/calls \
  -H "Idempotency-Key: unique-call-id-101" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl "http://localhost:3000/alerts?ruleId=REPLACE_WITH_ID&callId=REPLACE_WITH_CALLID"
```
